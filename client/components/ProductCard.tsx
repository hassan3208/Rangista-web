...
export default function ProductCard({
  ...
  const discountPercent = getDiscountForCollection(collection);
  const discountedPrice = applyDiscount(currentPrice, discountPercent);
  const finalPrice = discountPercent ? discountedPrice : currentPrice;
  ...
  const add = async () => {
    ...
    try {
      ...
      // Update local cart state (client-side reflection)
      addItem({ id, name, price: finalPrice, image, size, collection }, 1);
    } catch (error) {
      console.error("Error adding to cart:", error);
      alert("Failed to add item to cart. Please try again.");
    }
    ...
    <Card className="overflow-hidden relative">
      <CardHeader className="p-0">
        <div className="relative">
          ...
          {discountPercent > 0 && (
            <span className="absolute left-3 top-3 rounded bg-destructive text-destructive-foreground px-2 py-1 text-xs font-semibold">
              -{discountPercent}%
            </span>
          )}
          <FavButton id={id} />
          ...
          <div className="text-right font-semibold">
            {discountPercent > 0 ? (
              <div className="flex flex-col items-end">
                <span className="text-xs text-muted-foreground line-through">{formatPKR(currentPrice)}</span>
                <span>{formatPKR(finalPrice)}</span>
              </div>
            ) : (
              <span>{formatPKR(currentPrice)}</span>
            )}
          </div>
        </div>
        <div className="mt-1 text-xs flex items-center gap-2">
