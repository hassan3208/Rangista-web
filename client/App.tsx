  ...
  useEffect(() => {
    ...
    if (performance.navigation.type === 1 && window.location.pathname !== "/") {
      ...
      if (confirmGoHome) {
        ...
        // stay on same page without reload
        window.history.pushState(null, "", window.location.href);
      }
    }
  }, []); // run once on real reload

  useEffect(() => {
    const handleReloadKeys = (e: KeyboardEvent) => {
      const path = location.pathname;
      // Intercept F5 or Ctrl+R
      if ((e.key === "F5" || (e.ctrlKey && e.key.toLowerCase() === "r")) && path !== "/") {
        e.preventDefault();
        const confirmGoHome = window.confirm("Do you want to go home?");
    ...
    window.addEventListener("keydown", handleReloadKeys);
    return () => window.removeEventListener("keydown", handleReloadKeys);
